...
export default function PersonalTraining() {
  ...
  const handleToggleFavorite = async (exerciseId) => {
    ...
    try {
      if (isFavorite) {
        ...
        setFavoriteExerciseIds(favoriteExerciseIds.filter(id => id !== exerciseId));
      } else {
        await axios.post(`http://127.0.0.1:8000/api/favorite-exercice`, { membre_id: memberId, exercice_id: exerciseId });
        setFavoriteExerciseIds([...favoriteExerciseIds, exerciseId]);
      }
    } catch (error) {
  ...
  const filteredByLevel = currentLevel
    ? filteredData.filter((item) => item.level === currentLevel)
    : filteredData;
  ...
  if (loading) {
    return <div>
      <span class="loaderfavorites"></span>
      <span class="loaderfavorites"></span>
    </div>;
  }
